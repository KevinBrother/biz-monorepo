...
function getCatalogDependencies() {
  ...
  const catalogSection = content.match(/catalog:\s*([\s\S]*?)(?=\n\S|\n$|$)/)?.[1]
  ...
  Array.from(catalogDeps).sort().forEach(dep => {
    console.log(`   📦 ${dep}`)
  })
...
function verifyCatalogUsage() {
  ...
  packageFiles.forEach(file => {
    ...
    const usageRate = possibleUsageInFile > 0 ? (catalogUsageInFile / possibleUsageInFile * 100).toFixed(1) : '0.0'
    const status = usageRate === '100.0' ? '✅' : usageRate === '0.0' ? '❌' : '⚠️'
    
    console.log(`${status} ${file}: ${catalogUsageInFile}/${possibleUsageInFile} (${usageRate}%)`)
  ...
  const overallRate = totalPossibleUsage > 0 ? (totalCatalogUsage / totalPossibleUsage * 100).toFixed(1) : '0.0'
